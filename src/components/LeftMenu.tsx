...
export default function LeftMenu() {
  const { friends, friendName, setFriends, updateFriendId, updateFriendName } =
    useFriendStore();

  useEffect(() => {
    const fetchFriends = async () => {
      try {
        const friendsList: User[] = await getFriends();
        setFriends(friendsList);

        // Initialiser le premier ami dans le store si la liste n'est pas vide
        if (friendsList.length > 0) {
          updateFriendId(friendsList[0].id);
    ...
    fetchFriends();
  }, [setFriends, updateFriendId, updateFriendName]);
      ...
      <div className="profile-section">
        ...
        <div className="profile-container">
          ...
          <p
            ...
            onClick={() =>
              navigator.clipboard.writeText(friendName || "Tirstan")
            }
          >
            {friendName || "Tirstan"}
          </p>
            ...
            <ul>
              {friends.map((friend) => (
                <li
                  ...
                  onClick={() => {
                    updateFriendId(friend.id);
                    updateFriendName(friend.username);
                  }}
                >
                  {friend.username}
                </li>
